...
names <- sapply(strsplit(files, "/|_"), `[`, 2) %>% unlist() %>% tolower() %>% unique()


for (a in names) {
  print(paste("Processing author:", a))
  
  author_files <- files[str_detect(files, regex(a, ignore_case = TRUE))]
  print(paste("Author files:", author_files))
  
  t <- lapply(author_files, read_file) %>% paste(collapse = "\n\n")
  
  # Construct file name
  filename <- ""
  if (str_detect(a, "navalny")) {
    if (str_detect(a, "free")) {
      filename <- "corpus_clean/navalny_free.txt"
    } else if (str_detect(a, "jailed")) {
      filename <- "corpus_clean/navalny_jailed.txt"
    }
  } else if (str_detect(a, "oleg")) {
    if (str_detect(a, "free")) {
      filename <- "corpus_clean/oleg_free.txt"
    } else if (str_detect(a, "jailed")) {
      filename <- "corpus_clean/oleg_jailed.txt"
    }
  } else {
    filename <- paste0("corpus_clean/", a, ".txt")
  }
...
texts = tibble(
  path = files, 
  author = str_extract_all(files, "(?<=corpus_clean/).*?(?=\\.txt)") %>% unlist(), 
...
sample_independent <-function(tokenized_df, n_samples,sample_size,text_var="word",folder="corpus_sampled/",overwrite=T) {
  ...
  # Calculate the min number of observations across all authors (because of Oleg_free)
 # min_obs <- min(table(tokenized_df$author))
  
shuff <- tokenized_df  %>% 
  group_by(author) %>%
  do(sample_n(., size = nrow(.))) %>%  # reshuffle rows for each author
  #sample_n(min(min_obs, n_samples * sample_size)) %>%
  group_by(author) %>% 
...
sample_independent(corpus_tokenized %>% filter(!author %in% c('navalnaya', 'oleg_free')),
                   sample_size=4500,
                   text_var = "word",
                   n_samples = 2)
...
## true random sampling
sample_independent(corpus_chunks %>% filter(!author %in% c('navalnaya', 'oleg_free')),
                   sample_size=160,
                   text_var = "line",
                   n_samples = 2)
...
# sample_independent(tokenized_df = corpus_tokenized %>% filter(!author %in% c('navalnaya', 'oleg_free')),
#                    sample_size = 4500,
#                    n_samples = 2,
#                    text_var = "word",
#                    overwrite = T,
...
sample_independent(
  tokenized_df = corpus_tokenized %>% filter(!author %in% c('navalnaya', 'oleg_free')),  # Exclude specific author
...
# Remove the 'corpus_sampled/' prefix from the author names
umap_plot_data$author <- str_remove(umap_plot_data$author, "^corpus_sampled/")
...
sample_independent(
  tokenized_df = corpus_chunks %>% filter(!author %in% c('navalnaya', 'oleg_free')),  # Exclude a specific author
...
# Remove the 'corpus_sampled/' prefix from the author names
visualization_data$author <- str_remove(visualization_data$author, "^corpus_sampled/")
...
sample_independent(
  tokenized_df = corpus_chunks %>% filter(!author %in% c('navalnaya', 'oleg_free')),  # Exclude specific author
  sample_size = 160, # Size of each sample
  n_samples = 2, # Number of samples to generate
  text_var = "line", # Analyze lines
  overwrite = TRUE, # Overwrite existing files
...
# Remove the 'corpus_sampled/' prefix from the author names
visualization_data_2$author <- str_remove(visualization_data_2$author, "^corpus_sampled/")
  
  # Create a scatter plot using ggplot
ggplot(visualization_data_2, aes(x, y, color = author)) + 
  geom_text(aes(label = author), size = 6) + 
  theme_bw() + 
  guides(color = "none") + 
  scale_color_viridis_d() + 
...
# Create a tibble for visualization
visualization_data_3 <- tibble(
  x = projection$layout[, 1],
  y = projection$layout[, 2],
  author = str_remove_all(rownames(dt), "_[0-9]*?$")
)%>% filter(author!="corpus_sampled/sobol")

# Remove the 'corpus_sampled/' prefix from the author names
visualization_data_3$author <- str_remove(visualization_data_3$author, "^corpus_sampled/")
  
  # Create a scatter plot using ggplot
ggplot(visualization_data_3, aes(x, y, color = author)) + 
  geom_text(aes(label = author), size = 5) + 
  theme_bw() + 
  guides(color = "none") + 
  scale_color_viridis_d() + 
  labs(title = "UMAP projection, cosine delta, 200 MF words, 4.5k words samples")
```
...
for(i in 1:iters) {
  ...
  ## true random sampling
sample_independent_opt(corpus_chunks %>% filter(!author %in% c('navalnaya', 'oleg_free')),
                   sample_size=150,
                   text_var = "line",
                   n_samples = 2)
